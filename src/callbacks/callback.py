...
def callbackHandler(call):
    # Since wishlist can be added from inline which don't have chat id
    if call.data.startswith('addWishlist'):
        magnetKey = call.data[15:]
        dbSql.addWishlist(call.from_user.id, magnetKey)

        bot.answer_callback_query(call.id, language['wishlistAdded']['english'])
